...
public class YoutubeAudioService {
    ...
    public String processYoutubeData(String uuid) throws IOException, InterruptedException {
        ...
        if (metaData == null) {
            System.out.println("❗ 메타데이터 파싱 실패: null 반환");
        } else {
            System.out.println("✅ 메타데이터 제목: " + metaData.getTitle());
            System.out.println("✅ 메타데이터 채널명: " + metaData.getChannel());
        }
    ...
    public YoutubeMetaData loadMetaData(String uuid) throws IOException {
        ...
        System.out.println("✅ 메타데이터 파일 발견: " + jsonPath.toAbsolutePath());

        String json = Files.readString(jsonPath);
        System.out.println("✅ 메타데이터 파일 내용 (앞부분):\n" + json.substring(0, Math.min(json.length(), 500)));
